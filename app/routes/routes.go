...
var sysNoticeControllers = controllers.NewSysNoticeController()             // 通知控制器
...
func InitRoutes(engine *gin.Engine) {
	...
	api := engine.Group("/api")
	{
		...
		// WebSocket 通知通道
		public.GET("/ws/notifications", sysNoticeControllers.Connect)

		authOnly := api.Group("")
		authOnly.Use(middleware.JWTAuthMiddleware())
		{
			myNotice := authOnly.Group("/sysNotice/my")
			{
				myNotice.GET("/list", sysNoticeControllers.ListMine)
				myNotice.PUT("/read", sysNoticeControllers.MarkRead)
				myNotice.GET("/unreadCount", sysNoticeControllers.UnreadCount)
			}

			adminNotice := authOnly.Group("/sysNotice")
