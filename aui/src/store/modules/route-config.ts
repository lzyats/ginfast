...
function appendLocalNoticeRoute(data: any[]) {
    const noticeRoute = {
        id: 0,
        parentId: 10,
        path: "/system/notice",
        name: "SystemNotice",
        component: "system/notice/notice",
        redirect: "",
        meta: {
            title: "通知管理",
            hide: false,
    ...
    const systemRoute = data.find((item) => item.path === "/system" || item.name === "system");
    if (!systemRoute) {
        data.push(noticeRoute);
        return;
    }

    const children = Array.isArray(systemRoute.children) ? systemRoute.children : [];
    if (!children.some((item: any) => item.path === noticeRoute.path || item.name === noticeRoute.name)) {
        children.push(noticeRoute);
    }
    systemRoute.children = children;
}
