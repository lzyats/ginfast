...
class Http {
    ...
    private static rejectPendingRequests(error: unknown) {
        Http.requests.forEach(item => item.reject(error));
        Http.requests = [];
    }
    ...
    private httpInterceptorsRequest(): void {
        Http.axiosInstance.interceptors.request.use(
            async (config: HttpRequestConfig): Promise<any> => {
                ...
                const whiteList = ["/refreshToken", "/login", "/captcha/id", "/captcha/image", "/captcha/verify", "/config/get"];
                            ...
                            if (!Http.isRefreshing) {
                                ...
                                useUserStoreHook()
                                    ...
                                    .then((res: any) => {
                                        ...
                                        Http.requests.forEach(item => item.resolve(token));
                                        Http.requests = [];
                                    })
                                    .catch((error: unknown) => {
                                        Http.rejectPendingRequests(error);
